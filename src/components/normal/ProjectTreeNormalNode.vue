...
import type { ProjectTreeNormalNodeProps, NodeData } from '@/utils/interface.js';
...
const {
    ...
    allowDrag,
    allowDrop,
} = defineProps<ProjectTreeNormalNodeProps>();

// 设置节点的不可枚举属性和默认值
Object.entries({
    _isVisible: data._isVisible ?? true,
    _isCurrent: data._isCurrent ?? false,
    _isChecked: data._isChecked ?? false,
    _isExpanded: data._isExpanded ?? true,
    _isExpandedOld: data._isExpandedOld ?? true,
...
<template>
    <div class="project-tree-normal-node" :class="{
    ...
    }" v-show="data._isVisible" ref="projectTreeNormalNodeRef" :draggable="draggable && allowDrag(data)"
        @dragstart.self="onDragStart($event, data, projectTreeNormalNodeRef)"
        @dragend.self="onDragEnd($event, data, projectTreeNormalNodeRef)">
        <div class="project-tree__content" @click.self="onNodeClick($event, data, projectTreeNormalNodeRef)"
            ...
            <!-- 展开图标 -->
            <div v-if="expandIcon" class="project-tree_icon expand-icon" :style="{
            ...
            <!-- 选择框 -->
            <div v-if="checkbox" class="project-tree_icon checkbox"
                @click="onCheckboxClick($event, data, projectTreeNormalNodeRef)">
                <slot name="checkbox" :data="data" :size="checkboxSize">
                    <!-- 选中 -->
                    <svg v-if="data._isChecked" viewBox="0 0 1024 1024" fill="currentColor" :width="checkboxSize"
                        :height="checkboxSize">
                        <path
                    ...
                    <!-- 半选 -->
                    <svg v-else-if="getAllChildren(data).some(child => child._isChecked)" viewBox="0 0 1024 1024"
                        fill="currentColor" :width="checkboxSize" :height="checkboxSize">
                        <defs>
                    ...
                    <!-- 未选 -->
                    <svg v-else viewBox="0 0 1024 1024" fill="currentColor" :width="checkboxSize"
                        :height="checkboxSize">
                        <path
            ...
            <expand-transition>
                <div class="project-tree__children" v-show="data._isExpanded">
                    <template v-for="node in data[childrenKey]" :key="node">
                        <project-tree-normal-node v-if="node ? true : (console.warn(`未渲染节点, 无效的节点数据(${node})`), false)"
                            :parent="data" :data="node" :id-key="idKey" :label-key="labelKey"
                            :children-key="childrenKey" :current-data="currentData"
                            :highlight-current="highlightCurrent" :level="level + 1" :expand-icon-hold="expandIconHold"
                            :expand-icon="expandIcon" :expand-icon-size="expandIconSize" :checkbox="checkbox"
                            :checkbox-size="checkboxSize" :node-icon="nodeIcon" :node-icon-size="nodeIconSize"
                            @expand-click="onExpandClick" @checkbox-click="onCheckboxClick"
                            @node-click.self="onNodeClick" @node-dblclick="onNodeDblclick"
                            @node-right-click="onNodeRightClick" :draggable="draggable" :allow-drag="allowDrag"
                            :allow-drop="allowDrop" @start="onDragStart" @enter="onDragEnter" @leave="onDragLeave"
                            @over="onDragOver" @dropped="onDropped" @end="onDragEnd">
