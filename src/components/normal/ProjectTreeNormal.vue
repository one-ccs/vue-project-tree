...
import type { ProjectTreeNormalProps, DroppedExtraData, NodeData } from '@/utils/interface.js';
...
const {
    ...
    dropOffset = 8,
} = defineProps<ProjectTreeNormalProps>();
...
const projectTreeNormalRef = ref<any>(null);
// 虚拟根节点
...
const onNodeClick = (event: MouseEvent, data: NodeData, nodeElement: HTMLElement) => {
    if (event.ctrlKey && event.shiftKey) {
        ...
        if (checkbox) {
            if (!data._isChecked && !data._children?.every(child => child._isChecked)) clearMultipleList();
...
<template>
    <div
        ...
        class="vue-project-tree-normal"
        ref="projectTreeNormalRef"
    >
